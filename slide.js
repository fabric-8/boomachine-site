/* ==========================================================================
   ...
   that stop the <a>'s native drag are kept):
     ...
     go          released while armed: the knob lands at the very end in
                 120 ms (v3: .is-go, not the 420 ms spring), the fill HOLDS
                 the armed intensity (no fade; a deep-red floor rises under
                 the whole track), the label is gone, and the page navigates
                 to knob.href 150 ms after the release (60 ms if the knob was
                 already at the end) — v2m waited 650 ms and the client
                 called it a delay. Enter / Space while armed (or at the
                 end) does the same; arriving at the end by ArrowRight arms.
     click       a plain click / tap on the knob (< 6 px of movement) or on
                 the track: the knob runs from where it is to the end over
                 ~600 ms (ease-in-out, shorter from further along), the fill
                 following it in real time, arming as it crosses the
                 threshold, then the page goes 250 ms later — ~850 ms from
                 the click to the navigation (v2m: ~1.35 s). A press on the knob
...
(function (global) {
  ...
  /* v3: the client on v2m — "there's a delay, it's janky; if I drag it all
     the way right the page isn't loading right away". The 650 ms go delay
     is gone. A RELEASE past the threshold navigates as soon as the knob has
     landed: the knob takes a 120 ms ease to the end (.is-go in slide.css,
     not the 420 ms spring) and the page goes at GO_RELEASE_MS; if the knob
     is already at the end it goes after one beat (GO_LANDED_MS). A CLICK
     auto-slides in AUTO_MS and then goes after GO_CLICK_MS. The light stays
     armed throughout. */
  var GO_RELEASE_MS = 140;            /* release while armed -> navigation (measured ~150 with the timer slack) */
  var GO_LANDED_MS = 60;              /* ...when the knob is already at the end */
  var GO_CLICK_MS = 250;              /* after the click's auto-slide        */
  var AUTO_MS = 600;                  /* the click's auto-slide, full length */
  ...
  /* the knob eases to the very end, the fill holds the armed intensity (a
     ...
     after GO_RELEASE_MS / GO_CLICK_MS the page navigates to knob.href. */
  /* `how`: "release" (a pointer release / Enter while armed: the fast
     landing, GO_RELEASE_MS) or "click" (the end of the auto-slide: the knob
     is already there, GO_CLICK_MS). */
  function go(how) {
    if (going) return;
    going = true;
    var fromClick = how === "click";
    stopAuto();
    setArmed(true);
    track.classList.add("done");
    measure();
    var travel = max - x;                        /* how far the knob still has to go */
    /* the fast landing: .is-go swaps the 420 ms spring for a 120 ms ease
       (slide.css). It goes on BEFORE .dragging comes off, so the transform
       never sees the slow transition. */
    knob.classList.add("is-go");
    ...
    var delay = fromClick ? GO_CLICK_MS : (travel > 1 && !reduce.matches ? GO_RELEASE_MS : GO_LANDED_MS);
    goAt = now();
    /* (tscale is 1 unless QA slowed the clock: then the delay stretches with it) */
    goTimer = global.setTimeout(function () { goTimer = 0; navAt = now(); global.location.href = knob.href; }, delay / tscale);
  }
  var goAt = 0, navAt = 0, upAt = 0;           /* QA: the timing of the flow */
  ...
  function autoSlide() {
    ...
    measure();
    if (reduce.matches) { x = max; place(max); setArmed(true); go("click"); return; }
    auto = true;
  ...
  function release() {
    ...
    if (armed && x > max * THRESH) { go("release"); }
  ...
  knob.addEventListener("keydown", function (e) {
    ...
    if (e.key === "ArrowRight" || e.key === "ArrowLeft") {
      ...
      e.preventDefault();
      atEnd = x >= max - 0.5;
      if (armed || atEnd) { upAt = now(); go("release"); } else autoSlide();
    }
  });
  ...
  global.booSlide = {
    ...
    go: function (how) { go(how || "release"); },         /* QA: what a release while armed does */
    sweep: sweep,
