...
(function (global) {
  ...
  /* v3: ~150 motes on desktop, ~100 on a phone (v2g: 100 / 75).
     v7: 240 / 140 — the client: "more particles/dust to the top" — and
     POOL_K of them are pool motes: they live in the red pool ABOVE the
     sign (from 30 px over the top edge down to the title's middle), drift
     slowly, and respawn in that band instead of under the hand, so the
     top of the hero carries more dust than the rest of the halo. */
  var COUNT = 240, COUNT_M = 140, POOL_K = 0.32, STEP = 1000 / 30;
  ...
  /* v7: the pool band, in hero px: from 30 px above the top edge to the
     middle of the title's text box (the red pool the sign hangs in) */
  function poolBand() {
    var t = G && G.title, h = G && G.hero;
    var lo = -30, hi = t && h ? (t.y - h.top) + t.h * 0.5 : 120;
    return { lo: lo, hi: Math.max(hi, lo + 60) };
  }
  function seed(p, first) {
    var a = Math.random() * Math.PI * 2, u = Math.sqrt(Math.random());
    if (p.pool) {
      /* a pool mote: anywhere in the band, x within the halo's core; it
         climbs slowly, so it lingers in the band */
      var b = poolBand();
      p.x = halo.x + (Math.random() * 2 - 1) * halo.rx * 0.82;
      p.y = first ? b.lo + Math.random() * (b.hi - b.lo) : b.hi - 6 + Math.random() * 24;
      /* hotter than the halo's dust: it is IN the light — the pink sprite
         more often, a higher floor on the alpha, a few bigger and closer */
      p.s = 0.9 + Math.pow(Math.random(), 1.5) * 3.6;
      p.a = 0.48 + Math.random() * 0.52;
      p.vx = (Math.random() - 0.5) * 9;
      p.vy = -(1.5 + Math.random() * 4.5);        /* 1.5-6 px/s: ~20-60 s across the band */
      p.w = 0.15 + Math.random() * 0.5; p.ph = Math.random() * 6.283;
      p.k = Math.random() < 0.6 ? 1 : 0;
  ...
  function sizeMotes() {
    ...
    var n = mw < 520 ? COUNT_M : COUNT;
    if (motes.length !== n) {
      motes = [];
      for (var i = 0; i < n; i++) motes.push(seed({ pool: i < n * POOL_K }, true));
    }
  }
  ...
  function stepMotes(dt, ts) {
    for (var i = 0; i < motes.length; i++) {
      ...
      p.y += p.vy * dt;
      if (p.pool) { if (p.y < -30 || p.x < -30 || p.x > mw + 30) seed(p, false); }
      else if (p.y < halo.y - halo.ry * 1.2 || p.x < -30 || p.x > mw + 30) seed(p, false);
    }
  }
  ...
  global.booFx = {
    ...
    /* v7: the two mote sprites, for the story's dust (dust.js) — the same look */
    sprites: function () { if (!sprites) sprites = [sprite(255, "58,44"), sprite(255, "128,104")]; return sprites; },
    alphaAt: alphaAt,
    motes: function () { return motes.length; }
  };
