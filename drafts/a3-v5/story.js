/* ==========================================================================
   Boo Machine — the motion of the page   (v5: deterministic, cheap)

   1. THE HERO SCRUB (v4, kept). One timeline scrubbed over ~1.2 viewports:
      the unit / spill / light planes rise 18svh and grow to 1.04, the title
      rises 6svh, the slide zone rises 28svh and fades by 60 %, the bulb
      (--rl-k) leans up then hands over. The island comes on at 62 %.
   2. THE STAGE — ONE SCROLL-DRIVEN TIMELINE. .stage-wrap is pinned for
      3 x 100svh. The stage progress p (0..1) is the only clock: the morph
      from hand i to i+1 runs over WIN[i] (p .30–.40 and .63–.73) as a pure
      function of p — the WebGL displacement reads t = (p − a) / (b − a),
      the DOM fallback crossfades with the same t — so scrolling back and
      forth can never leave two hands overlapping or a morph half-done: at
      any p there is exactly one defined state. The display crossfades, the
      copy blocks and the red flare (peaked mid-window) are tweens on the
      same timeline, smoothed by ScrollTrigger's `scrub: 0.4` (one tween on
      the timeline, not separate time-based tweens).
   ...
   4. THE VIDEOS. Only the clip of the current state plays; a clip is
      display:none unless it is (about to be) visible — it is switched on
      from the RAW trigger progress a margin ahead of its crossfade, so it
      has ~300 ms to land before it is seen; clips 2 and 3 are preload=none
      until then. Paused off screen (IO) and on visibilitychange.
   5. REFRESH POLICY. ScrollTrigger auto-refreshes on resize only
      (ignoreMobileResize); the page refreshes ONCE when the fonts and the
      stage's images have landed. Nothing refreshes during a scroll.
   6. REDUCED MOTION: no ScrollTrigger at all — no scrub, no pin, no morph.

   QA hooks — window.booStory:
     .set(i)          scroll to the middle of state i (0..2)
     .state           the current (discrete) state   .p   the timeline progress
     .to(what, p)     scroll so the hero ('hero') or the stage ('stage')
                      trigger is at progress p (0..1)
     .hero / .pin     the ScrollTrigger instances    .tl  the stage timeline
...
(function (global) {
  ...
  /* the morph windows, in stage progress: hand i -> i+1 over WIN[i] */
  var WIN = [[0.30, 0.40], [0.63, 0.73]];
  var MID = [0.15, 0.515, 0.86];                  /* a p safely inside each state */
  ...
  /* which clips have to be in the DOM at raw progress p (a margin ahead of
     their crossfade, so a clip lands before it is seen) */
  var MARGIN = 0.06;
  function showClips(p) {
    for (var k = 0; k < 3; k++) {
      var v = clips[k]; if (!v) continue;
      var a = k === 0 ? -1 : WIN[k - 1][0] - MARGIN, b = k === 2 ? 2 : WIN[k][1] + MARGIN;
      var on = p >= a && p <= b;
  ...
  /* ---------- 2. the state as a function of p --------------------------- */
  /* where(p): {i: from, j: to, t: 0..1 inside the window (0 outside)} */
  function where(p) {
    for (var k = 0; k < WIN.length; k++) {
      var a = WIN[k][0], b = WIN[k][1];
      if (p < a) return { i: k, j: k, t: 0 };
      if (p < b) return { i: k, j: k + 1, t: (p - a) / (b - a) };
    }
    return { i: 2, j: 2, t: 0 };
  }
  ...
  function setDiscrete(i) {
    if (i === cur) return;
    cur = i;
    copies.forEach(function (c, j) { c.classList.toggle("is-on", j === i); if (j === i) c.removeAttribute("aria-hidden"); else c.setAttribute("aria-hidden", "true"); });
  ...
  /* the hands at p: exactly one defined picture */
  function applyHands(p) {
    var w = where(p), k;
  ...
  var heroST = null, pinST = null, enterST = null, tl = null, refreshes = 0;
  var proxy = { p: 0 };
  ...
  function buildScroll() {
    ...
    /* ONE timeline, duration 1 == the stage progress p */
    gsap.set(copies, { opacity: function (i) { return i === 0 ? 1 : 0; }, y: 0 });
    tl = gsap.timeline({
      defaults: { ease: "none" },
      scrollTrigger: {
        trigger: stage, start: "top top", end: "+=300%", pin: true, anticipatePin: qs.get("apin") === "0" ? 0 : 1,
        pinType: qs.get("pin") === "transform" ? "transform" : "fixed",
        pinSpacing: true, scrub: 0.4, invalidateOnRefresh: false,
        onUpdate: function (self) { showClips(self.progress); },
        onRefresh: function (self) { refreshes++; showClips(self.progress); applyHands(tl ? tl.progress() : self.progress); },
        onEnter: warm, onEnterBack: warm
      }
    });
    /* the clock: everything that is not a tween reads proxy.p */
    tl.to(proxy, { p: 1, duration: 1, onUpdate: function () { applyHands(proxy.p); } }, 0);
    WIN.forEach(function (w, k) {
      var a = w[0], b = w[1], m = (a + b) / 2, half = (b - a) / 2;
      /* the copy: out over the first half, in over the second */
      /* every tween is an explicit fromTo: a `to` records its start from
         the DOM when the playhead first reaches it, and a jump straight to
         p = 1 would record the wrong one (seen: a clip stuck at 0) */
      tl.fromTo(copies[k], { opacity: 1, y: 0 }, { opacity: 0, y: -18, duration: half, ease: "power2.in", immediateRender: false }, a);
      tl.fromTo(copies[k + 1], { opacity: 0, y: 26 }, { opacity: 1, y: 0, duration: half, ease: "power3.out", immediateRender: false }, m);
      /* the display: a 60 % crossfade centred on the morph */
      var ca = m - half * 0.6, cd = half * 1.2;
      tl.fromTo(clips[k], { opacity: 1 }, { opacity: 0, duration: cd, immediateRender: false }, ca);
      tl.fromTo(clips[k + 1], { opacity: 0 }, { opacity: 1, duration: cd, immediateRender: false }, ca);
      /* the flare: peaked mid-window, on the room and on the phone */
      [stageFlare, flare].forEach(function (el) {
        if (!el) return;
        tl.fromTo(el, { opacity: 0 }, { opacity: 1, duration: half, ease: "power2.out", immediateRender: false }, a);
        tl.fromTo(el, { opacity: 1 }, { opacity: 0, duration: half, ease: "power2.in", immediateRender: false }, m);
      });
    });
  ...
  function killScroll() {
    ...
    gsap.set(copies.concat(videos, [stageFlare, flare]).filter(Boolean), { clearProps: "transform,opacity" });
  ...
  /* ONE refresh, when the fonts and the stage's images are in (the hero's
  ...
  /* ...and never while the page is being scrolled: a refresh mid-pin re-measures
     and re-pins, which is a visible jump. It waits for a quiet 250 ms. */
  ...
  global.booStory = {
    set: function (i) { return this.to("stage", MID[Math.max(0, Math.min(2, i | 0))]); },
    get state() { return cur; },
    get p() { return tl ? tl.progress() : 0; },
    get hero() { return heroST; }, get pin() { return pinST; }, get tl() { return tl; },
    ...
    hands: function () {
      return { layers: layers.map(function (l) { return l ? +getComputedStyle(l).opacity : null; }),
               ...
               where: where(tl ? tl.progress() : 0) };
    },
    videos: videos, layers: layers, WIN: WIN,
