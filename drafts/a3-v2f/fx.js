/* =========================================================================
   ...
   1. THE TITLE'S REFLECTION  (.d-refl, one canvas per .lbox)
      ...
      blurred 6 px. Redrawn only on resize / image load — never per frame. The
      slide with --px/--py is a CSS transform, exactly like .d-spec.

   2. DUST MOTES  (.fx-motes, one canvas over the hero)
      v2f: ~68 soft particles (52 on a phone) drifting up and sideways, their opacity multiplied by
...
(function (global) {
  ...
  function drawRefl() {
    ...
    f.addColorStop(0, "rgba(0,0,0,.10)");
    f.addColorStop(0.55, "rgba(0,0,0,.52)");
  ...
  /* v2f: the client asked for roughly twice the dust. 34 -> 68 (26 -> 52 on a
     phone), with a wider size spread so the extra particles do not read as one
     more even layer of the same speck. The draw is one drawImage per mote from
     a 24 px sprite with `lighter`, at 30 fps — see the README for the measured
     cost of the second 34. */
  var COUNT = 68, STEP = 1000 / 30;
