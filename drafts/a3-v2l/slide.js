/* ==========================================================================
   Boo Machine — slide to join   (v2l: the furnace port)

   The pointer / capture / drag logic, the keyboard support and the TestFlight
   URL are v2e-b's, carried through v2f–v2k unchanged (the block below the
   header, down to the `focus` listener, is byte-identical to v2i). What
   changed is what the drag DRIVES.

   v2k gave the light rays, a floor streak and life. The client on v2k: "the
   light looks a bit weird, the strings/streaks are weird, leave those out
   and just make it the noise-ish effect that is already there. Give it a
   stronger gradient from the button to the back, add a little bit more
   movement, make it a little bit hotter, and remove those strange light
   streaks." So v2l is v2k with the RAYS and the FLOOR STREAK gone (no
   shafts, no ray flares, no ray field), and what is left made steeper,
   hotter and busier:

     field    a STEEPER fall-off — pink-white inside ~10 px of the knob's
              edge, saturated red by ~40, deep red by ~120, near-black by
              ~220, a faint warm haze past that (two inverse-square terms,
              softening radii 22 and 90 px) — MULTIPLIED by the haze, so the
              hot zone has turbulent structure instead of a flat wash;
     haze     three octaves of value noise drifting left (~11 / 3.6 / 1.7
              px/s, v2k's × 1.8) and slowly up under a slow DOMAIN WARP
              (22–32 px), plus a fast fine octave (~17 px/s); rounder cells
              than v2k's; its contrast highest near the core (visible
              billows) and softer further back;
     bloom    one more octave (~14 px/s) whose contrast follows a slow random
              envelope — every 3–8 s the hot zone throws brighter turbulent
              patches for a second or two, like looking into a furnace port;
     core     #FFF0EA at the knob's rim with a thin amber ring (#FFB347) just
              outside it, breathing ±9 % over ~1.2 s with the 0.15 s
              micro-flicker kept; the heat-shimmer band at the knob's edge
              at ×1.4 amplitude (±1.26 px);
     dust     the same sparse motes with parallax, lit where the haze is
              bright (v2k lit them by the ray field).

   Red family only, no yellow flames, no tongues. No two layers share a
   clock: every period, speed and phase is drawn at random per page load.

   Unchanged from v2k: the canvas IS the track's box and every frame is drawn
   ...
   .release() / .reset() / .bloom(peak) / .flame (stats)
   ========================================================================== */
(function (global) {
  "use strict";
  ...
  var LIGHT = (function () {
    ...
    function paint(S) {
      ...
      var inten = S.inten, done = S.done, burst = S.burst, bl = bloom;
      var coreK = 0.62 * (1 + 2 * (pulse - 1));                  /* the source term follows the pulse (±18 %) */
      for (x = 0; x < W; x++) {
        var xc = INSET + (x + 0.5) * CX;
        ...
        colK[x] = (0.34 + 0.34 * Math.exp(-d1 / 70)) * (1 - 0.6 * Math.exp(-d1 / 8));
    ...
    function draw(c, S) {
      ...
      var ci = inten * (S.cooling ? Math.max(0, 1 - S.shift / 60) : 1) * pulse;
    ...
    function bloomNow(pk) {
      if (!clk) return 0;
      clk.bloomT0 = simT; clk.bloomUp = 0.5; clk.bloomHold = 0.6; clk.bloomDown = 1.4;
      clk.bloomPk = pk > 0 ? Math.min(1, pk) : 1; clk.bloomNext = simT + 4 + rnd() * 4;
      return clk.bloomPk;
    }
    return { name: "light", resize: resize, reset: reset, step: step, draw: draw, busy: busy, heat: heatL, staticFrame: staticFrame,
             bloom: bloomNow,
             stats: function () {
               return { dust: dust.length, bloom: +bloom.toFixed(3), pulse: +pulse.toFixed(3), t: +simT.toFixed(2), cells: FW + "x" + FH, hzCols: hzCols };
             } };
  })();
  R = LIGHT;
